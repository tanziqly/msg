...
const workSlides = [
  ...
  { src: gallery[6]!, competencies: expertCompetencies[4]! },
] as const
...
function VariantClassic() {
  ...
  const bottomRight = workSlides[4]! // "Технологическое развитие"
...
function TeamPhotoCard({
  ...
  return (
    <div
      tabIndex={0}
      className="group relative aspect-video overflow-hidden bg-slate-100 focus-visible:ring-2 focus-visible:ring-blue-600 focus-visible:outline-none sm:aspect-[16/10]"
    >
      <img
        src={src}
        alt={alt}
...
function CompetencyOverlay({
  ...
  return (
    <div className="absolute inset-0 flex items-end overflow-y-auto bg-slate-950/10 opacity-0 transition-opacity duration-500 group-focus-within:opacity-100 group-hover:opacity-100">
      <div className="absolute inset-0 bg-gradient-to-t from-slate-950/88 via-slate-950/45 to-slate-950/10" />
      <div className="relative w-full p-3.5 text-white sm:p-7">
        <p className="mb-1 text-[9px] font-semibold tracking-[0.2em] text-blue-200 uppercase sm:mb-2 sm:text-[11px] sm:tracking-[0.24em]">
          {group.eyebrow}
        </p>
        <h3 className="mb-1.5 text-sm leading-tight font-medium sm:mb-4 sm:text-xl">
          {group.title}
        </h3>
        <ul className="grid gap-1 sm:grid-cols-2 sm:gap-2">
          {group.points.map((point) => (
            <li
              key={point}
              className="flex items-start gap-1.5 text-[11px] leading-4 text-slate-100 sm:gap-2 sm:text-sm sm:leading-5"
            >
              <span className="mt-0.5 shrink-0 text-blue-300 sm:mt-1">→</span>
              {point}
            </li>
          ))}
        </ul>
